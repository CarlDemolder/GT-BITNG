...
uint32_t temp_char_add(ble_temperature_service_t *p_cus, const ble_temperature_service_init_t *p_cus_init)
{
    ...
    err_code = sd_ble_gatts_characteristic_add(p_cus->service_handle, &char_md, &attr_char_value, &p_cus->temp_char_handles);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
uint32_t temp_char_update(ble_temperature_service_t *p_cus, uint8_t *new_temp_char_array)
{
    ...
    // Update database.
    err_code = sd_ble_gatts_value_set(p_cus->conn_handle, p_cus->temp_char_handles.value_handle, &gatts_value);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
