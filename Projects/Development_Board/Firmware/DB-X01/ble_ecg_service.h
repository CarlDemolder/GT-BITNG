...
#define BLE_ECG_SERVICE_DEF(_name) static ble_ecg_service_t _name; NRF_SDH_BLE_OBSERVER(_name ## _obs, BLE_HRS_BLE_OBSERVER_PRIO, ble_ecg_service_on_ble_evt, &_name)
...
{
    ecg_service_evt_handler_t evt_handler;                    /**< Event handler to be called for handling events in the ECG Service. */
    uint8_t ecg_char[250];                                    /**< Initial custom value array to store the ECG voltage data */
