...
#define BLE_CONFIGURATION_SERVICE_DEF(_name) static ble_configuration_service_t _name; NRF_SDH_BLE_OBSERVER(_name ## _obs, BLE_HRS_BLE_OBSERVER_PRIO, ble_configuration_service_on_ble_evt, &_name)
...
typedef enum
{
    ...
    CONFIGURATION_SERVICE_EVT_WRITE,                                    /**< Configuration Service write event. */
...
{
    configuration_service_evt_handler_t evt_handler;                    /**< Event handler to be called for handling events in the Custom Service. */
    uint8_t settings_char[10];                                          /**< Characteristic to communicate the settings for the patch */
    uint8_t response_char[2];                                           /**< Characteristic to communicate the response from the patch */
    uint8_t crc_char[2];                                                /**< Characteristic to communicate the crc value to ensure data integrity*/
...
uint32_t settings_char_update(ble_configuration_service_t *p_cus, uint8_t *new_setting_char_array);
uint32_t response_char_update(ble_configuration_service_t *p_cus, uint8_t *new_response_char_array);
uint32_t crc_char_update(ble_configuration_service_t *p_cus, uint8_t *new_crc_char_array);
