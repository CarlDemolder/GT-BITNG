...
static struct BLE_ECG_Service_Control_Struct control;
...
uint32_t ble_ecg_service_initialize(ble_ecg_service_t *p_cus, const ble_ecg_service_init_t *p_cus_init)
{
    ...
    control.error_code = ecg_service_ecg_char_add(p_cus, p_cus_init);
    if (control.error_code != NRF_SUCCESS)
    {
    ...
    control.error_code = ecg_service_instant_ecg_char_add(p_cus, p_cus_init);
    if (control.error_code != NRF_SUCCESS)
    {
        NRF_LOG_INFO("DID NOT CREATE INSTANT ECG VALUE CHARACTERISTIC PROPERLY");
        return control.error_code;
    }
}
...
uint32_t ecg_service_ecg_char_add(ble_ecg_service_t *p_cus, const ble_ecg_service_init_t *p_cus_init)
{
    ...
    attr_char_value.init_len = sizeof(uint8_t)*ECG_SERVICE_ECG_CHAR_LENGTH;
    attr_char_value.init_offs = 0;
    attr_char_value.max_len = sizeof(uint8_t)*ECG_SERVICE_ECG_CHAR_LENGTH;

    control.error_code = sd_ble_gatts_characteristic_add(p_cus->service_handle, &char_md, &attr_char_value, &p_cus->ecg_char_handles);
    if (control.error_code != NRF_SUCCESS)
    {
        return control.error_code;
    }
...
uint32_t ecg_service_ecg_char_write(ble_ecg_service_t *p_cus, uint8_t *new_ecg_char_array)
{
    ...
    control.error_code = sd_ble_gatts_value_set(p_cus->conn_handle, p_cus->ecg_char_handles.value_handle, &gatts_value);
    if (control.error_code != NRF_SUCCESS)
    {
        return control.error_code;
    }
...
/**@brief Function for adding the Instant ECG characteristic.
 *
 * @param[in]   p_cus        ECG Service structure.
 * @param[in]   p_cus_init   Information needed to initialize the service.
 *
...
{
    NRF_LOG_INFO("ecg_service_instant_ecg_char_add");
    // Local Variables to the function
    ...
    attr_md.read_perm = p_cus_init->instant_ecg_char_attr_md.read_perm;
    attr_md.write_perm = p_cus_init->instant_ecg_char_attr_md.write_perm;
    attr_md.vloc = BLE_GATTS_VLOC_STACK;
    attr_md.rd_auth = 0;
    attr_md.wr_auth = 0;
    attr_md.vlen = 0;
    ...
    attr_char_value.p_uuid = &ble_uuid;
    attr_char_value.p_attr_md = &attr_md;
    attr_char_value.init_len = sizeof(uint8_t)*ECG_SERVICE_INSTANT_ECG_CHAR_LENGTH;
    attr_char_value.init_offs = 0;
    attr_char_value.max_len = sizeof(uint8_t)*ECG_SERVICE_INSTANT_ECG_CHAR_LENGTH;

    control.error_code = sd_ble_gatts_characteristic_add(p_cus->service_handle, &char_md, &attr_char_value, &p_cus->instant_ecg_char_handles);
    if (control.error_code != NRF_SUCCESS)
    {
        return control.error_code;
    }
...
uint32_t ecg_service_instant_ecg_char_write(ble_ecg_service_t *p_cus, uint8_t *new_instant_ecg_char_data)
{
    NRF_LOG_INFO("ecg_service_instant_ecg_char_update"); 
    if (p_cus == NULL)
    {
        return NRF_ERROR_NULL;
    }
    ...
    if ((p_cus->conn_handle != BLE_CONN_HANDLE_INVALID)) 
    {
        ...
        hvx_params.handle = p_cus->instant_ecg_char_handles.value_handle;
        hvx_params.type = BLE_GATT_HVX_NOTIFICATION;
        hvx_params.offset = gatts_value.offset;
        hvx_params.p_len = &gatts_value.len;
        hvx_params.p_data = gatts_value.p_value;
...
static void _ecg_service_on_write(ble_ecg_service_t *p_cus, ble_evt_t const *p_ble_evt)
{
    ...
    // Check if the INSTANT ECG value CCCD has been written and if the value is the appropriate length, i.e 2 bytes.
    if ((p_evt_write->handle == p_cus->instant_ecg_char_handles.cccd_handle) && (p_evt_write->len == 2))
    {
        // CCCD written, call application event handler
        if (p_cus->evt_handler != NULL)
        {
            ...
            if (ble_srv_is_notification_enabled(p_evt_write->data))
            {
                NRF_LOG_INFO("ECG_SERVICE_EVT_INSTANT_ECG_CHAR_NOTIFICATION_ENABLED");
                evt.evt_type = ECG_SERVICE_EVT_INSTANT_ECG_CHAR_NOTIFICATION_ENABLED;
            }
            else
            {
                NRF_LOG_INFO("ECG_SERVICE_EVT_INSTANT_ECG_CHAR_NOTIFICATION_DISABLED");
                evt.evt_type = ECG_SERVICE_EVT_INSTANT_ECG_CHAR_NOTIFICATION_DISABLED;
            }
            // Call the application event handler.
            p_cus->evt_handler(p_cus, &evt);
        }
